...
<section class="page-section mt-5">
    <div class="container">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title text-uppercase">DAFTAR ({{ $loker->judul . " - " . $loker->perusahaan->nama }})</h5><hr />
                    @if (count($errors) > 0)
                    ...
                    <ul class="nav nav-tabs" id="nakerTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="nakerdata-tab" data-bs-toggle="tab" data-bs-target="#nakerdata" type="button" role="tab" aria-controls="nakerdata" aria-selected="true">Data Pribadi</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="nakerdata-tab" data-bs-toggle="tab" data-bs-target="#nakercv" type="button" role="tab" aria-controls="nakercv" aria-selected="false">Curiculum Vitae</button>
                        </li>
                    </ul>

                    <form id="nakerForm" method="POST" action="{{ $naker->exists ? route('applylokernaker', $naker->nakerid) : route('applyloker') }}" class="form-horizontal form-material m-t-40 needs-validation" enctype="multipart/form-data" onsubmit="return validateAll();">
                        @csrf
                        @if($naker->exists)
                        {{ method_field('PUT') }}
                        @endif
                        ...
                        <input type="hidden" name="lokerid" id="lokerid" value="{{ $loker->lokerid }}">
                            ...
                            <div class="tab-pane fade" id="nakercv" role="tabpanel" aria-labelledby="nakercv-tab">
                                ...
                                <div class="mb-5">
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-md-6">
                                            <button onclick="javascript:addRiwayatPendidikan();" type="button" id="btnaddriwayatpendidikan" class="btn btn-sm btn-info float-end">Tambah</button> 
                                        </div>
                                    </div>
                                ...
                                <div class="mb-5">
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-md-6">
                                            <button onclick="javascript:addRiwayatKerja();" type="button" id="btnaddriwayatkerja" class="btn btn-sm btn-info float-end">Tambah</button> 
                                        </div>
                                    </div>
                                ...
                                <div class="mb-5">
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-md-6">
                                            <button onclick="javascript:addSertifikat();" type="button" id="btnaddserifikat" class="btn btn-sm btn-info float-end">Tambah</button> 
                                        </div>
                                    </div>
...
<script>
    var $validator = $("#nakerForm").validate({
        ignore: [],
        errorClass: 'd-block is-invalid invalid-feedback',
        rules: {
            namalengkap : {
                required: true
            },
    ...
    $(document).ready(function() {
        ...
        $('#provinsiid').select2().on('change', function() {
            var url = "{{ route('apply.kota', ':parentid') }}";
        ...
        $('#kotaid').select2().on('change', function() {
            var url = "{{ route('apply.kecamatan', ':parentid') }}";
        ...
        $('#kecamatanid').select2().on('change', function() {
            var url = "{{ route('apply.kelurahan', ':parentid') }}";
    ...
    $( "#modalrwytpddnamainstitusi" ).autocomplete({
        source: function( request, response ) {
            $.ajax( {
                url: "{{ route('apply.searchriwayatpendidikan') }}",
                dataType: "json",
                data: {
                    term: request.term,
                    type: "1"
    ...
    $( "#modalsrtbidangkeahlian" ).autocomplete({
        source: function( request, response ) {
            ...
            $.ajax( {
                url: "{{ route('apply.searchbidangkeahlian') }}",
                dataType: "json",
                data: {
                    term: request.term,
    ...
    function loadDataRiwayatKerja() {
        ...
        $.each( v_filter, function( p_key, p_value ) {
            ...
            @if(!$isshow)
            html += '<button onclick="javascript:hapusRiwayatKerja('+p_value.indexriwayatkerja+');" type="button" id="btnaddsubgolpokok" class="mx-1 btn btn-sm btn-danger float-end">Hapus</button>';
            html += '<button onclick="javascript:editRiwayatKerja('+p_value.indexriwayatkerja+');" type="button" id="btnaddsubgolpokok" class="mx-1 btn btn-sm btn-warning float-end">Ubah</button>';
            @endif
            html += '</div>';
            html += '</div>';
            html += '<div class="row">';
    ...
    function loadDataRiwayatPendidikan() {
        ...
        $.each( v_filter, function( p_key, p_value ) {
            ...
            html += '<div class="col-md-4">';
            @if(!$isshow)
            html += '<button onclick="javascript:hapusRiwayatPendidikan('+p_value.indexriwayatpendidikan+');" type="button" id="btndltriwayatpendidikan" class="mx-1 btn btn-sm btn-danger float-end">Hapus</button>';
            html += '<button onclick="javascript:editRiwayatPendidikan('+p_value.indexriwayatpendidikan+');" type="button" id="btneditriwayatpendidikan" class="mx-1 btn btn-sm btn-warning float-end">Ubah</button>';
            @endif
            html += '</div>';
            html += '</div>';
            html += '<div class="row">';
    ...
    function loadDataSertifikat() {
        ...
        $.each( v_filter, function( p_key, p_value ) {
            ...
            html += '<div class="col-md-4">';
            html += '<button onclick="javascript:hapusSertifikat('+p_value.indexsertifikat+');" type="button" id="btndltsertifikat" class="mx-1 btn btn-sm btn-danger float-end">Hapus</button>';
            html += '<button onclick="javascript:editSertifikat('+p_value.indexsertifikat+');" type="button" id="btneditsertifikat" class="mx-1 btn btn-sm btn-warning float-end">Ubah</button>';
            html += '</div>';
            html += '</div>';
            html += '<div class="row">';
            html += '<div class="col-md-12">';
    ...
    function addBidangKeahlian(bidangkeahlian, bidangkeahlianid = "", idx) {
        let html = '';
        html += '<span class="badge rounded-pill bg-primary badge-info mb-1 mx-1" style="line-height: 2;" id="modalsrtbidangkeahlianbadge'+idx+'">';
        ...
        html += '<button type="button" class="btn-close" aria-label="Close" onclick="hapusBidangKeahlian('+idx+');"></button>';
        html += '</span>';
